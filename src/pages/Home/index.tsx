...
const Home = () => {
  ...
  const searchResults = async (order: string, orientation: string) => {
    if (query.trim()) {
      console.log('Entrou')
      isLoading(true)
      setLastResult({
      ...
      const searchResult = await searchPhotos(query,order, orientation, page)
  ...
  useEffect(() => {
    ...
    searchResults(order, orientation)
  }, [page])
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.filter}>
          <button onClick={() => {searchResults('relevant', orientation)}}>Mais Relevantes</button>
          <button onClick={() => {searchResults('latest', orientation)}}>Mais Recentes</button>

          <button onClick={() => {searchResults(order, 'portrait')}}>Vertical</button>
          <button onClick={() => {searchResults(order, 'landscape')}}>Horizontal</button>
        </div>
